          ...
          ClipRRect(
            child: SvgPicture.asset(
              ...
              colorFilter: ColorFilter.mode(
                Theme.of(context).colorScheme.fontColor.withValues(alpha: 0.7),
                BlendMode.srcIn,
              ),
            ),
          ),
          ...
          children: [
            Row(
              children: [
                model!.codAllowed == '1'
                    ? Expanded(
                        child: getImageWithHeading(Assets.cod, cod, context),
                      )
                    : Container(width: 0),
                Expanded(
            ...
            model!.codAllowed == '1'
                ? Consumer<PaymentProvider>(
                    builder: (context, paymentProvider, _) {
                      final minAmount = paymentProvider.minCodAmount;
                      final maxAmount = paymentProvider.maxCodAmount;

                      // Only show if at least one limit is set
                      ...
                      if (minAmount != null && maxAmount != null) {
                        message =
                            '${'COD_AVAILABLE_BETWEEN'.translate(context: context)} ${DesignConfiguration.getPriceFormat(context, minAmount)} ${'AND_LBL'.translate(context: context)} ${DesignConfiguration.getPriceFormat(context, maxAmount)}';
                      } else if (minAmount != null) {
                        message =
                            '${'COD_MIN_AMOUNT'.translate(context: context)} ${DesignConfiguration.getPriceFormat(context, minAmount)}';
                      } else if (maxAmount != null) {
                        message =
                            '${'COD_MAX_AMOUNT'.translate(context: context)} ${DesignConfiguration.getPriceFormat(context, maxAmount)}';
                      }
