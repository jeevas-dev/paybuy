...
class StateItem extends State<ProductDetail> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Fetch payment settings for COD min/max amounts
    context.read<PaymentProvider>().getPaymentSettings();
  ...
  Future<void> getProductDetails() async {
    if (isNetworkAvail) {
      try {
        ...
        ApiBaseHelper()
            .postAPICall(getProductApi, parameter)
            .then(
              (getdata) async {
                ...
                if (!error) {
                  ...
                  for (
                    ...
                    for (int j = 0; j < variantAttributes.length; j++) {
                      index = widget.model!.attributeList![i]
                          .getIdList()
                          .indexWhere(
                    ...
                    attributes.add(
                      AttributeSelection(
                        ...
                        selectedId: widget.model!.attributeList![i].getId(
                          index,
                        ),
                      ),
                    );
      ...
      if (isNetworkAvail) {
        try {
          if (context.read<ProductDetailProvider>().isLoadingmore) {
            ...
            ApiBaseHelper()
                .postAPICall(getProductFaqsApi, parameter)
                .then(
                  (getdata) {
                    bool error = getdata['error'];
                    if (!error) {
                      ...
                      context
                          ...
                          .toList();
                      context.read<ProductDetailProvider>().isLoadingmore =
                          true;
    ...
    return Positioned.fill(
      child: Padding(
        ...
        child: Align(
          ...
          child: indicator == '1'
              ? SvgPicture.asset(DesignConfiguration.setSvgPath(Assets.vag))
              : indicator == '2'
              ? SvgPicture.asset(DesignConfiguration.setSvgPath(Assets.nonvag))
              : const SizedBox(),
        ),
      ),
    );
                      ...
                      Future.delayed(const Duration(seconds: 1)).then((_) {
                        if (context.read<ProductDetailProvider>().error ==
                          ...
                          if (context
                            ...
                            setSnackbar(
                              'Cart Clear successfully ...!'.translate(
                                context: context,
                              ),
                              context,
                            );
    ...
    try {
      ...
      if (isNetworkAvail) {
        try {
          ...
          ApiBaseHelper()
              .postAPICall(getRatingApi, parameter)
              .then(
                (getdata) {
                  bool error = getdata['error'];
                  String? msg = getdata['message'];
    ...
    try {
      ...
      if (isNetworkAvail) {
        try {
          ...
          ApiBaseHelper()
              .postAPICall(setFavoriteApi, parameter)
              .then(
                (getdata) {
                  bool error = getdata['error'];
                  String? msg = getdata['message'];
                  if (!error) {
                    ...
                    context.read<FavoriteProvider>().addFavItem(
                      from == 1 ? productList[index] : widget.model,
                    );
                    setSnackbar(msg!, context);
                  } else {
                    setSnackbar(msg!, context);
  ...
  Widget getSilverAppBar(Product model) {
    return SliverAppBar(
      ...
      flexibleSpace: FlexibleSpaceBar(background: _slider()),
    );
  }
                  ...
                  Column(
                    ...
                    children: <Widget>[
                      ...
                      attributes.isNotEmpty &&
                          ...
                          : const SizedBox(),
                      getDivider(2, context),
                      context.read<PromoCodeProvider>().promoCodeList.isNotEmpty
                          ? SaveExtraWithOffers(update: update)
                          : const SizedBox.shrink(),
                      context.read<PromoCodeProvider>().promoCodeList.isNotEmpty
                          ? getDivider(2, context)
                          : const SizedBox(),
                      ProductMoreDetail(model: widget.model, update: update),
        ...
        Wrap(
          ...
          children: List.generate(
            ...
            (index) => _buildAttributeValueItem(
              attributeSelection.oneAttributeList,
              index,
              attributeSelection,
            ),
          ),
  ...
  int getSelectedVariantIndex() {
    List<String> selectedAttributeIds = attributes
        .map((e) => e.selectedId.trim())
        .toList();
    int variantIndex = 0;
                ...
                (getdata) {
                  ...
                  if (error) {
                    ...
                    if (getdata['data'] != null) {
                      ...
                      codDeliveryCharges =
                          getdata['data']['delivery_charge_with_cod']
                              .toString();
                      prePaymentDeliveryCharges =
                          getdata['data']['delivery_charge_without_cod']
                              .toString();
                      //
                    } else {
                      deliveryDate = '';
    ...
    try {
      ...
      if (isNetworkAvail) {
        try {
          ...
          ApiBaseHelper()
              .postAPICall(checkDeliverableApi, parameter)
              .then(
                (getdata) {
                  bool error = getdata['error'];
                  String? msg = getdata['message'];
  ...
  Widget _buildVariantProductActionBar() {
    ...
    return Container(
      ...
      child: Selector<CartProvider, bool>(
        selector: (_, provider) =>
            provider.cartList.any((element) => element.varientId == variant.id),
        builder: (context, isInCart, child) => Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            if (isInCart) ...[
  ...
  Widget _buildAddToCartButton() {
    return Expanded(
      child: InkWell(
        onTap: () async {
          if (!context.read<CartProvider>().isProgress) {
            addToCart(qtyController.text, false, true, widget.model!);
          }
        },
        child: SizedBox(
          height: heightAnimation.value,
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        ...
        hintStyle: Theme.of(context).textTheme.titleSmall!.copyWith(
          color: Theme.of(context).colorScheme.fontColor.withValues(alpha: 0.8),
        ),
        suffix: GestureDetector(
          ...
          child: Text(
            ...
            style: const TextStyle(color: colors.primary, fontFamily: 'ubuntu'),
          ),
        ),
      ),
    );
