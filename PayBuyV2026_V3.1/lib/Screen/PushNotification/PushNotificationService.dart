...
class PushNotificationService {
  ...
  static Future<void> handleNotification(RemoteMessage notification) async {
    ...
    // Get title and body from either notification.notification or notification.data
    var title =
        notification.notification?.title ?? data['title']?.toString() ?? '';
    var body =
        notification.notification?.body ?? data['body']?.toString() ?? '';
    ...
    if (type == 'chat') {
      ...
      if (converstationScreenStateKey.currentState?.mounted ?? false) {
        ...
        return; // Don't show notification if user is viewing the chat
      } else {
        HasUnreadChatRepository.setChatUnread(true);
        //senders_name
        createChatNotification(
    ...
    // Handle ticket status notifications
  ...
  static Future<void> _navigation(Map<String, String> payload) async {
    ...
    switch (type) {
      ...
      case 'categories':
        ...
        final catIndex = catList.indexWhere((c) => c.id == id);
