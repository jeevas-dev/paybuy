...
class HomePageProvider extends ChangeNotifier {
  ...
  Future<void> getSliderImages() async {
    ...
    await HomeRepository.fetchSliderImages().then((result) {
      if (!result['error']) {
        List<Model> tempList = [];
  ...
  Future<void> getCategories(BuildContext context) async {
    ...
    var parameter = {CAT_FILTER: 'false'};

    await HomeRepository.fetchCategories(parameter: parameter).then((result) {
      ...
      if (!error) {
        ...
        if (result.containsKey('popular_categories')) {
          ...
          popularList = (data as List)
              .map((data) => Product.fromCat(data))
              .toList();

          if (popularList.isNotEmpty) {
            Product pop = Product.popular(
              'Popular',
              DesignConfiguration.setSvgPath(Assets.popular),
            );
  ...
  Future<void> getSections({
    ...
    await HomeRepository.fetchSections(parameter: parameter).then((result) {
      if (!result['error']) {
  ...
  Future<void> getOfferImages() async {
    ...
    await HomeRepository.fetchOfferImages().then((result) {
      if (!result['error']) {
        List<Model> tempList = [];
  ...
  Future getFav(BuildContext context) async {
    ...
    if (isNetworkAvail) {
      if (context.read<UserProvider>().userId != '') {
        ...
        apiBaseHelper
            .postAPICall(getFavApi, parameter)
            .then(
              (getdata) {
                ...
                if (!error) {
                  ...
                  if (msg != 'No Favourite(s) Product Are Added') {
                    setSnackbar(
                      'No Favourite(s) Product Are Added'.translate(
                        context: context,
                      ),
                      context,
                    );
