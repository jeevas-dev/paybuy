...
class PaymentProvider extends ChangeNotifier {
  ...
  Future<void> getdateTime(BuildContext context, Function updateNow) async {
    ...
    if (isNetworkAvail) {
      ...
      try {
        ...
        if (!error) {
          ...
          flutterwave = payment['flutterwave_payment_method'] == '1'
              ? true
              : false;
          ...
          myfatoorah = payment['myfaoorah_payment_method'] == '1'
              ? true
              : false;
  ...
  // Fetch only payment settings (for COD min/max amounts)
  Future<void> getPaymentSettings() async {
    bool isNetworkAvail = await isNetworkAvailable();
    if (isNetworkAvail) {
      try {
        var parameter = <String, dynamic>{TYPE: PAYMENT_METHOD};
        Map<String, dynamic> result =
            await PaymentRepository.getDataTimeSettings(parameter: parameter);
        bool error = result['error'];
  ...
  Future<Map<String, dynamic>> payWithPayStack({
    ...
    try {
      ...
      if (response.status) {
        ...
        return {'error': false, 'message': response.message, 'status': true};
      }
    } catch (e) {
      return {'error': true, 'status': false, 'message': e.toString()};
    }
  }
  ...
  Future<String> getFlutterWaveGatewayLink({
    ...
    try {
      var parameter = {AMOUNT: paymentAmount, ORDERID: unioqueOrderId};
