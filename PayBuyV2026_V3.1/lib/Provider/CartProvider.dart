    ...
    if (isNetworkAvail) {
      try {
        ...
        if (!error) {
          ...
          totalPrice = 0;
          if (IS_SHIPROCKET_ON == '0' && context.read<AppSettingsCubit>().getProductDeliverabilityType() != 'group_wise') {
            if (!ISFLAT_DEL) {
      ...
      if (isNetworkAvail) {
        try {
          ...
          if (!error) {
            ...
            deliveryCharge =
                (double.tryParse(data['delivery_charge'].toString()) ?? 0);
            print("Product Deliverability Type: ${context.read<AppSettingsCubit>().getProductDeliverabilityType()}");
            if (IS_SHIPROCKET_ON == '0' && context.read<AppSettingsCubit>().getProductDeliverabilityType() != 'group_wise') {
              // if(context.read<AppSettingsCubit>().getProductDeliverabilityType() != 'group_wise') {
              if (!ISFLAT_DEL) {
                print("oriPrice: $oriPrice");
                print("freeAmt: ${addressList[selectedAddress!].freeAmt!}");
                if ((oriPrice) <
                    double.parse(addressList[selectedAddress!].freeAmt!)) {
                  ...
                  deliveryCharge = 0;
                  print("deliveryCharge1111: $deliveryCharge");
                }
              } else {
                if ((oriPrice) < double.parse(MIN_AMT!)) {
                  deliveryCharge = double.parse(CUR_DEL_CHR!);
                  print("deliveryCharge2222: $deliveryCharge");
                } else {
                  deliveryCharge = 0;
                  print("deliveryCharge3333: $deliveryCharge");
                }
              }
              ...
              totalPrice = deliveryCharge + oriPrice;
            // }
            // print("Else condition deliveryCharge: $deliveryCharge - oriPrice: $oriPrice");
            // totalPrice = deliveryCharge + oriPrice;
            }
             else {
              print("Else condition deliveryCharge:1111 $deliveryCharge - oriPrice: $oriPrice");
              totalPrice = deliveryCharge + oriPrice;
            }
