...
class PaymentRepository {
  ...
  fetchAvailablePaymentMethodsAndPaymentGatewayIDs({
    ...
    try {
      var systemSetting = await ApiBaseHelper().postAPICall(
        getSettingApi,
        parameter,
      );
  ...
  static Future<Map<dynamic, dynamic>> payWithPaytm({
    ...
    try {
      Map<dynamic, dynamic> response = await ApiBaseHelper()
          ...
          .then((paytmResponseFromAPI) async {
            var paytmResponse = await PaytmPaymentsAllinonesdk()
                .startTransaction(
                  paytmMerchantID,
                  paytmOrderID,
  ...
  static Future<Map> getPhonePeDetails({
    ...
    try {
      var responseData = await ApiBaseHelper()
          .postAPICall(getPhonePeDetailsApi, <String, dynamic>{
            'type': type,
            'mobile': mobile,
